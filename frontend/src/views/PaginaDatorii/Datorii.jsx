...
const Datorii = () => {
    ...
    const [dataImprumut, setDataImprumut] = useState(new Date().toISOString().split('T')[0]);
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(`http://localhost:8080/api/debts/user/${userId}`);
    ...
    const handleSubmit = async (event) => {
        ...
        if (denumire && suma && dataImprumut && dataScadenta) {
            try {
                const response = await axios.post(`http://localhost:8080/api/debts/user/${userId}`, {
                    name: denumire,
                    amount: suma,
    ...
    const handlePlatireDatorie = async (index) => {
        const updatedDatorie = { ...datorii[index], isPaid: true };
        try {
            await axios.put(`http://localhost:8080/api/debts/${updatedDatorie.id}`, updatedDatorie);
        ...
        <div className="datorii-container d-flex flex-column justify-content-center align-items-center p-3 mb-2 bg-success text-white">
            ...
            <div className="vizualizare-datorii-container">
                ...
                <table>
                    ...
                    <tbody>
                        {Array.isArray(datorii) && datorii
                            .filter((datorie) => datorie.isPaid)
                            .map((datorie, index) => (
                                <tr key={index}>
                                    <td>{datorie.name}</td>
                                    <td>{datorie.amount}</td>
                                    <td>{new Date(datorie.dateIncurred).toLocaleDateString("ro-RO")}</td>
                                    <td>{new Date(datorie.dueDate).toLocaleDateString("ro-RO")}</td>
                                </tr>
                            ))}
                    </tbody>
                </table>
                        ...
                        {Array.isArray(datorii) && datorii
                            ...
                            .map((datorie, index) => (
                                <tr key={index}>
                                    ...
                                    <td>{new Date(datorie.dueDate).toLocaleDateString("ro-RO")}</td>
                                    <td>
                                        <button onClick={() => handlePlatireDatorie(index)}>Marchează ca plătită</button>
                                    </td>
                                </tr>
                            ))}
                    </tbody>
            ...
            >
                <div className="adauga-datorii-container">
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <input type="text" id="suma" value={suma} onChange={(e) => setSuma(e.target.value)} />

                        <label htmlFor="dataImprumut">Data imprumut:</label>
                        <input type="date" id="dataImprumut" value={dataImprumut} onChange={(e) => setDataImprumut(e.target.value)} />

                        <label htmlFor="dataScadenta">Data scadenta:</label>
                        <input type="date" id="dataScadenta" value={dataScadenta} onChange={(e) => setDataScadenta(e.target.value)} />
